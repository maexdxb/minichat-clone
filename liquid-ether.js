 ...
 * Final Fix: Event Listeners on parent and refined glow.
 */
...
class LiquidEther {
    constructor(container, options = {}) {
        ...
        this.options = {
            ...
            cursorSize: options.cursorSize || 80,
            ...
            autoSpeed: options.autoSpeed || 0.8,
            autoIntensity: options.autoIntensity || 1.2,
            autoResumeDelay: options.autoResumeDelay || 1000,
            dissipation: 0.94 // Disappear faster for a cleaner look
        };
    ...
    init() {
        ...
        // --- SHADERS ---
        const face_vert = `attribute vec3 position; varying vec2 uv; void main(){ uv = position.xy * 0.5 + 0.5; gl_Position = vec4(position, 1.0); }`;
        ...
        // Refined Glow Shader
        const color_frag = `precision highp float; uniform sampler2D velocity; uniform sampler2D palette; varying vec2 uv; void main(){ vec2 vel = texture2D(velocity, uv).xy; float lenv = length(vel); float glow = smoothstep(0.01, 0.4, lenv); glow = pow(glow, 1.5); // Thinner, sharper glow vec3 c = texture2D(palette, vec2(glow, 0.5)).rgb; gl_FragColor = vec4(c * glow, 1.0); }`;
        ...
        // --- INTERACTION: Listen to parent to bypass pointer-events: none ---
