 ...
 * Premium Version: Balanced glow, localized interaction, and robust events.
 */
...
class LiquidEther {
    constructor(container, options = {}) {
        ...
        this.options = {
            mouseForce: options.mouseForce || 150,
            ...
            autoSpeed: options.autoSpeed || 1.2,
    ...
    init() {
        ...
        // --- SHADERS ---
        const face_vert = `varying vec2 uv; void main(){ uv = uv; gl_Position = vec4(position, 1.0); }`;
        // Simplifizierter Vertex-Shader für r167 Kompatibilität (uv wird automatisch durch PlaneGeometry gereicht)
        ...
        const color_frag = `precision highp float; uniform sampler2D velocity; uniform sampler2D palette; varying vec2 vUv; void main(){ vec2 vel = texture2D(velocity, vUv).xy; float lenv = length(vel); float glow = smoothstep(0.0, 0.5, lenv); glow = pow(glow, 0.8); vec3 c = texture2D(palette, vec2(glow, 0.5)).rgb; gl_FragColor = vec4(c * glow, 1.0); }`;

        const divergence_frag = `precision highp float; uniform sampler2D velocity; uniform float dt; uniform vec2 px; varying vec2 vUv; void main(){ float x0 = texture2D(velocity, vUv-vec2(px.x, 0.0)).x; float x1 = texture2D(velocity, vUv+vec2(px.x, 0.0)).x; float y0 = texture2D(velocity, vUv-vec2(0.0, px.y)).y; float y1 = texture2D(velocity, vUv+vec2(0.0, px.y)).y; float divergence = (x1 - x0 + y1 - y0) / 2.0; gl_FragColor = vec4(divergence / dt, 0.0, 0.0, 1.0); }`;
