 ...
 * Ultra-Stable Version: works with older and newer Three.js versions.
 */
...
class LiquidEther {
    constructor(container, options = {}) {
        ...
        this.options = {
            mouseForce: options.mouseForce || 180, // High force for visibility
            ...
            dissipation: 0.97 // Keep trail alive longer
        };
    ...
    init() {
        ...
        // Output Shader: High visibility
        const color_frag = `precision highp float; uniform sampler2D velocity; uniform sampler2D palette; varying vec2 vUv; void main() { float lenv = length(texture2D(velocity, vUv).xy); float glow = smoothstep(0.0, 0.4, lenv); glow = pow(glow, 0.7); vec3 c = texture2D(palette, vec2(glow, 0.5)).rgb; gl_FragColor = vec4(c * glow, 1.0); }`;
        ...
        const createPass = (frag, uniforms, vShader = vert) => {
            const scene = new THREE.Scene(); const camera = new THREE.Camera();
            const mat = new THREE.ShaderMaterial({ vertexShader: vShader, fragmentShader: frag, uniforms });
            scene.add(new THREE.Mesh(new THREE.PlaneGeometry(2, 2), mat));
            return { scene, camera, mat };
        };

        const advection = createPass(advection_frag, { velocity: { value: null }, dt: { value: this.options.dt }, dissipation: { value: this.options.dissipation }, fboSize: { value: new THREE.Vector2(fboW, fboH) } });
        const externalForce = createPass(externalForce_frag, { px: { value: cellScale }, force: { value: new THREE.Vector2() }, center: { value: new THREE.Vector2() }, scale: { value: new THREE.Vector2(this.options.cursorSize, this.options.cursorSize) } }, mouse_vert);
        externalForce.mat.transparent = true; externalForce.mat.blending = THREE.AdditiveBlending;
        const divergence = createPass(divergence_frag, { velocity: { value: null }, dt: { value: this.options.dt }, px: { value: cellScale } });
        const poisson = createPass(poisson_frag, { pressure: { value: null }, divergence: { value: fbos.div.texture }, px: { value: cellScale } });
        const pressure = createPass(pressure_frag, { pressure: { value: null }, velocity: { value: null }, px: { value: cellScale }, dt: { value: this.options.dt } });
        const output = createPass(color_frag, { velocity: { value: fbos.v0.texture }, palette: { value: paletteTex } });
